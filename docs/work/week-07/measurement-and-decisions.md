...
**판정**: 진단 기록표(117번째 줄)의 "채택" 결정이 실제 코드 동작으로 확인됨. `findLastSuccessfulProductList`가 `queryClient.getQueryCache()`에서 가장 최근 성공 쿼리를 렌더 중에 직접 조회하는 방식이라(로컬 state·ref 복사 없음) react-query 캐시가 데이터를 계속 소유하고, 필터 UI는 사용자가 시도한 새 값을 그대로 유지해 "무엇을 눌렀는지" 잃지 않는다.
