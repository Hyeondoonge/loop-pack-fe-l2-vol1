...
| After SHA       | `339e29b9133bd3e40669a1c96684c36ff1d7c102`("perf: suspense 경계 수정")                                                                                                                                                                                                                                                                                       |
| 실행            | `pnpm build && pnpm start` (개발 서버 측정 없음)                                                                                                                                                                                                                                                                                                             |
| URL             | `http://localhost:3000/` — 단 After 재측정 시점엔 3000번 포트에 별도 dev 서버가 상주 중이라 `http://localhost:3400/`에서 별도 production 인스턴스로 측정함(포트 외 조건 동일)                                                                                                                                                                                |
...
| `APP_ORIGIN`    | build와 runtime에 같은 값 — After 재측정은 `http://localhost:3400`(측정 인스턴스 origin). `RootLayout.tsx`의 미설정 시 throw는 After SHA 이전 어느 시점에 제거돼 이제 미설정이어도 build는 성공한다 — 단 `apiFetch.ts`는 변경 없어 미설정 시 런타임에 `${undefined}/api/home` 요청으로 깨진다(build 실패 → 런타임 실패로 실패 시점만 이동, 여전히 필수 설정) |
