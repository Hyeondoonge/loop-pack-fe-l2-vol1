...
**결정: 무개입.** 근본 원인(재정렬로 인한 DOM 노드 위치 이동)에 대해 별도 대응하지 않는다. 근거:

1. fallback ↔ 실제 콘텐츠 교체 과정에서 발생하는 CLS는 존재하지 않는다(53행, 5회 실측 모두 CLS 0으로 확인됨).
2. 재사용 노드(React `key` 재사용) 이동으로 인한 CLS는 의도된 동작으로 처리한다 — 정렬 변경은 사용자가 명시적으로 요청한 콘텐츠 재구성이고, 발제문 범위(fallback·실제 콘텐츠 교체 시점의 CLS)를 벗어나는 별개 상호작용이다.
3. 필터 변경 시 버튼 이동으로 인한 레이아웃 이동을 확인했으나, 심각도가 낮아 별도로 처리하지 않기로 결정했다.

검토했으나 채택하지 않은 대안:

- **View Transitions API / FLIP 애니메이션**: 카드 이동을 애니메이션으로 부드럽게 보여줄 수 있으나, Layout Instability API 점수 자체는 애니메이션 유무와 무관하게 그대로 잡힐 수 있어(수치 개선이 아니라 체감 개선) 무개입 결정과 별개로 실익이 확인되지 않았다.
- **정렬 결과를 새 배열로 교체하지 않고 DOM 순서를 유지한 채 `order` CSS 속성만 바꾸는 방법**: 실험하지 않음.
