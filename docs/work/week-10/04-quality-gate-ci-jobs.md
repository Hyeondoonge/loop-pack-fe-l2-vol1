...
- 상태: **작성 시점 결정** (2026-09-10, 워크플로 파라미터 2026-09-11 추가). 폰트 조치는 Docker 단계까지 보류. 아직 워크플로 변경 없음.
...
- 결정: **CI 게이트로 넣지 않는다.** 배포 후 운영 확인 항목으로 둔다.
...
- 과제 완료 기준 6개 항목(46~51번 줄)에 Lighthouse가 없다는 점도 함께 고려했다.
...
**`needs`를 걸지 않는 이유.** `e2e`에 `needs: quality`를 걸면 순차가 되어 `(68 − E) + (44 + B + E) = 112 + B`가 된다. `E` 값과 무관하게 병렬보다 느리다. 얻는 것은 "싼 검증이 실패할 때 비싼 E2E를 돌리지 않는다"인데, 그 대가로 성공하는 모든 실행이 매번 quality job 시간만큼 더 걸린다.
...
- 채택하지 않은 것: **단일 job 유지.** required 체크 이름이 하나뿐이라 E2E만 정책을 다르게 가져갈 수 없다. 지금은 둘 다 required지만, E2E에 flaky가 관측되면 시점을 옮겨야 하므로 이름을 분리해 둔다.
