...
- 상태: **작성 시점 결정** (2026-09-09). 아직 코드 없음.
...
6. [폴백을 없앤다](#6-폴백을-없앤다)
7. [`VERCEL_ENV` 교차검사는 넣지 않는다](#7-vercel_env-교차검사는-넣지-않는다)
8. [아직 확인하지 않은 것](#8-아직-확인하지-않은-것)
9. [검토 중 철회한 근거](#9-검토-중-철회한-근거)
...
현재 소비처는 두 곳이다. 둘 다 서버 실행 구간이라 `NEXT_PUBLIC_` 접두사가 필요 없다.

- `src/shared/api/apiFetch.ts:19` — `typeof window === 'undefined'` 분기 안의 self-fetch
- `src/_app/RootLayout.tsx:24` — `metadataBase`
...
- [Vercel — System environment variables](https://vercel.com/docs/environment-variables/system-environment-variables) — `VERCEL_ENV` · `VERCEL_URL` · `VERCEL_BRANCH_URL` 정의
...
- 코드: `src/shared/api/apiFetch.ts:19`, `src/_app/RootLayout.tsx:24`
