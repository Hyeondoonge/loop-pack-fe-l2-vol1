...
- 이번 주에 하지 않을 최적화와 이유 — 측정된 문제가 없으면 하지 않는다. 이번 주 diff에 성능 변경이 섞이면 회귀가 났을 때 폴더 이동 탓인지 가릴 수 없다.

| 하지 않을 것                       | 이유                                                                             | 착수 조건                             |
| ---------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------- |
| 캐시 정책 재조정(staleTime·gcTime) | 유지가 검증의 전제 — "hydration 후 클라 호출 0건"이 쿼리 키·옵션 동일성에 기댄다 | 전환 완료 후 신선도 요구가 달라질 때  |
| memo류(`React.memo`·`useMemo`)     | 현재 0곳. 카드 12개라 리렌더 비용 근거 없고, E단계에서 카드 구조가 막 바뀌었다   | Profiler에서 카드 리렌더가 보일 때    |
| 번들 분할(`next/dynamic`)          | 라우트 2개, 번들 크기 미측정. 슬라이스 경계가 생겨 나중에 하기 더 쉬워졌다       | 초기 로드에 불필요한 슬라이스 확인 시 |
| 이미지 튜닝(`sizes`·`priority`)    | `ProductCard`가 이미 `next/image` 사용. 어느 이미지가 LCP인지 모른다             | LCP 측정 후                           |
| 검색 debounce·프리페치             | 5주차 YAGNI 유지. 제출 시에만 URL 승격이라 막을 요청이 없다                      | —                                     |

# 에러 처리 표

| 실패 유형                         | 처리 위치                                          | Error Boundary로 전파하는가                  | 사용자 UI                                                                            | 재시도 방법                      | 이 경계를 선택한 이유                                                                                                                           |
| --------------------------------- | -------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| 상품 목록 조회 실패(5xx·네트워크) | `ProductListSection` — `useQuery`의 `isError` 분기 | **아니오** — `useQuery`가 에러를 값으로 반환 | 결과 영역만 `상품 목록을 불러오지 못했습니다.` + `다시 시도`. 필터 영역은 계속 렌더  | `refetch()` — 전체 새로고침 없음 | 필터를 바꾸면 성공할 수도 있는 실패다. 경계로 던지면 필터 컨트롤까지 함께 사라져 사용자가 스스로 빠져나올 수단을 잃는다                         |
| 잘못된 검색 조건(4xx)             | 같은 위치 — `error.status < 500` 분기              | **아니오**                                   | 서버 메시지(예: `요청 조건을 확인해주세요.`) + `필터 초기화`(재시도 버튼 없음)       | 없음 — 입력 교정이 복구 수단     | 같은 조건으로 재요청하면 같은 400이 돌아온다. 재시도 버튼은 사용자를 반복 실패에 가둔다                                                         |
| 예상하지 못한 렌더링 오류         | 홈: `HomePage`의 `ErrorBoundary` / **목록: 없음**  | 홈 **예** / 목록 **경계 자체가 없음**        | 홈: `HomeErrorFallback`(헤더 유지) / 목록: **Next 기본 오류 화면**(헤더·`main` 소실) | 양쪽 다 없음 — `reset` 미구현    | 홈은 데이터 실패와 렌더 오류가 같은 화면으로 수렴해도 무방하다. 목록은 경계를 두지 않아 화면 전체를 잃는다 — **미해결 공백**                    |
| 장바구니 행위의 비즈니스 오류     | **해당 없음**                                      | 해당 없음                                    | 해당 없음                                                                            | 해당 없음                        | 담기·찜은 Zustand `Set` 토글이라 실패 경로가 없다(`createIdSetStore`). 서버 반영·재고 검증·로그인 요구가 생기면 mutation 실패 처리가 필요해진다 |

# 실패 재현 결과

| 항목                         | 판정       | 남은 것                                                    |
| ---------------------------- | ---------- | ---------------------------------------------------------- |
| 1. 부분 표시 + 인라인 재시도 | 부분 충족  | 홈 fallback에 `resetErrorBoundary` 재시도 없음             |
| 2. 전파 기준과 표 일치       | 부분 충족  | `throwOnError` 미사용 — 훅 선택이 기준. 표와는 일치        |
| 3. `error.tsx` + `reset`     | **미충족** | `app/products/error.tsx` 부재 — 목록 렌더 오류에 경계 없음 |
| 4. 공통 에러 타입            | 부분 충족  | 네트워크 오류가 `ApiError`로 정규화되지 않음               |
| 5. 핸들러·콜백 오류          | 충족       | 해당 없음 — 서버 반영 시 `useMutation.onError`로           |
| 6. 로딩 범위 구분            | 충족       | `loading.tsx` 미채택 근거 기록 완료                        |

# 삭제 시나리오 자가 검증

#### 위시리스트 기능 통째 제거

검증방법: `grep -rn "wishlist\|Wishlist" app src`로 확인. 5개 파일이 잡히고, 그중 4개가 실제 대상이다.

검토 결과

- 영향 파일이 entities·features·widgets 세 레이어, 4개 파일에 걸쳐 있지만 grep 한 번으로 전부 찾았고 응집 실패(숨은 참조)는 없었다.
- 삭제 대상(폴더째)
  - `src/entities/wishlist/`(`model/wishlistStore.ts`)
  - `src/features/toggle-wishlist/`(`ui/WishlistToggleButton.tsx`)
- 삭제 후 수정 필요
  - `src/widgets/product-card/ui/ProductCardWithActions.tsx`
  - `**src**/widgets/**header**/ui/Header.tsx`

#### 신상품 뱃지를 상품 카드에 추가

검증방법: `Product` 타입·mock 데이터·두 API 응답 매핑 코드를 확인.

검토 결과

- 카드 표현을 entity 레이어 한 곳에 모은 설계가 실제로 성립함을 확인
- 수정 필요:
