...
| 운영 조건 | 결정 | 근거 |
| --- | --- | --- |
| 배포 대상이 Vercel이고, 리뷰용 Preview와 Production 두 환경만 필요하다 (현재) | **Vercel** | 저장소에 추가할 파일이 없고 Preview 배포·Instant Rollback을 플랫폼이 제공한다(1절). Docker를 쓰면 레지스트리와 컨테이너를 실행할 호스트를 따로 마련해야 한다(3.3). |
| 배포 대상이 Vercel이 아니다 — 사내 인프라, 컨테이너 플랫폼, 백엔드와 같은 배포 표준을 따라야 한다 | **Docker** | 산출물이 이미지라 실행 플랫폼을 가리지 않는다(1절). Vercel은 산출물이 플랫폼 안의 배포라 밖으로 옮길 수 없다. |
| 런타임을 정확히 고정해야 한다 — Node 패치 버전, OS 패키지, 실행 사용자 | **Docker** | 베이스 이미지가 `node:24.17.0-slim`으로 패치 버전까지 고정하고 실행 사용자도 `Dockerfile`에 적힌다(1·2절). Vercel은 `engines`로 메이저 버전만 지정한다. |
| 같은 빌드 산출물을 여러 환경에 그대로 올려야 한다 | **Docker.** 단 `NEXT_PUBLIC_` 값이 필요하면 환경별 이미지가 된다 | 서버 전용 값은 실행 때 `-e`로 바꾸므로 이미지 하나를 재사용할 수 있다. `NEXT_PUBLIC_` 값은 빌드 때 번들에 들어가 이미지에 고정된다(4절). |
| 플랫폼이 주입하는 값에 의존하지 않아야 한다 | **Docker** | 컨테이너에는 `VERCEL_ENV`·`VERCEL_URL`이 없고, `APP_ORIGIN`만으로 동작하는 것을 확인했다(4·5절). |
| 배포 빈도가 높고 운영 인력이 적다 | **Vercel** | Docker를 쓰면 이미지 빌드·태그·레지스트리 보관·롤백 대상 관리가 모두 직접 운영할 대상이 된다(3절). |

Docker를 도입한다면 이 기록에서 드러난 두 가지를 함께 처리해야 한다.

- 실행 시 `APP_ORIGIN` 누락은 컨테이너가 기동한 뒤 요청 500으로만 드러난다(5.2). 기동 시점 검증이나 헬스체크가 필요하다.
- `APP_ORIGIN` 하나가 컨테이너 내부 self-fetch 주소와 외부 공개 주소를 겸한다(4절). 컨테이너 포트와 공개 주소가 다르면 값을 분리해야 한다.
