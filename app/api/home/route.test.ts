...
describe('GET /api/home', () => {
  it('returns banner, categories, popular products, and new products', async () => {
    ...
    expect(body.popularProducts.map((product: { id: string }) => product.id)).toEqual(['p21', 'p11', 'p15', 'p8', 'p22', 'p30']);
    expect(body.newProducts.map((product: { id: string }) => product.id)).toEqual(['p26', 'p6', 'p27', 'p24', 'p1', 'p28']);
  });
