...
export const performanceLabProducts: PerformanceLabProduct[] = Array.from({ length: 24 }, (_, index) => ({
  id: `p${index + 1}`,
...
export function calculateCardPresentation(productId: string, selected: boolean) {
  const productSeed = Array.from(productId).reduce((sum, character) => sum + character.charCodeAt(0), selected ? 31 : 17);
