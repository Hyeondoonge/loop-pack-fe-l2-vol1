  ...
  return (
    <div className="checkout">
      ...
      <div className="section">
        <h2>주문 상품</h2>
        {cart.map((it) => (
          <OrderLineRow key={it.id} type="product" label={it.name} amount={it.price * it.quantity} thumbnail={it.thumbnail} option={it.option} quantity={it.quantity} />
        ))}
      </div>
      ...
      <div className="section">
        <h2>쿠폰</h2>
        <div className="row">
          <input type="text" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} placeholder="쿠폰 코드 (예: WELCOME5000)" />
      ...
      <div className="section">
        ...
        {appliedCoupon ? <OrderLineRow type="coupon" label="쿠폰 할인" amount={couponDiscount} isDiscount couponCode={appliedCoupon.code} /> : null}
      ...
      <div className="section">
        ...
        {PAST_ORDERS.map((o) => (
          <div key={o.id} className="line">
            ...
            <OrderStatusTag isPaid={o.status === 'paid'} isPreparing={o.status === 'preparing'} isShipped={o.status === 'shipped'} isDelivered={o.status === 'delivered'} isCancelled={o.status === 'cancelled'} />
          </div>
        ))}
      </div>
    </div>
